...
create_comparison_plot <- function(data, demographic_pair, fit_results_1, fit_results_0, k_fixed, 
  ...
  plot_title <- if(show_title) {
    var_titles <- list("GENDER" = "Gender", "AGE_binary" = "Age", "RELIGION_binary" = "Religiosity",
                       "INC" = "Income", "ALC" = "Alcohol Use")
...
# Create grid with ONLY the 5 demographic plots (no Overall plot in grid)
selected_vars <- c("GENDER", "AGE_binary", "RELIGION_binary", "INC", "ALC")
grid_plots <- list()

# Add the 5 demographic plots
...
# Arrange in grid - now 3 rows x 2 columns (5 plots)
if(length(grid_plots) == 5) {
  grid_combined <- grid.arrange(grobs = grid_plots, ncol = 2)
  
  # Save the grid
  ggsave("demand_curves_grid_with_overall.png", grid_combined, width = 12, height = 15, dpi = 300)
  cat("\n\n2x3 grid plot saved as 'demand_curves_grid_with_overall.png'\n")
}
